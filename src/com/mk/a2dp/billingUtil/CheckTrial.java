package com.mk.a2dp.billingUtil;
...
public class CheckTrial {
	...
	public final static String marketLink = "market://details?id=com.mk.a2dpLicence&c=apps";
	public final String appLink = "market://details?id=com.mk.a2dp&c=apps";
	//private final String licenseLink = "market://details?id=com.mk.iPlayAudioLicence&c=apps";
	public static final String fullVersionUri = "com.mk.a2dpLicence";
	public static final String marketUri = "com.mk.a2dp";
 ...
 // (arbitrary) request code for the purchase flow
		...
		if(!isAmazonBuild){
			if(inAppPurchased()){
		        ...
		        try
		        {
		        		...
		        		if(pckgInfo != null){
		        			String LicensedTo = getWhomLicenseTo();
		        			if(LicensedTo == null || LicensedTo.equalsIgnoreCase("all"))
		        				app_installed = true;
		        			else
		        				app_installed = checkGmailIdForLicense(LicensedTo);
		        		}
		        		else
		        			app_installed = false;
		        }
	...
	private static boolean checkGmailIdForLicense(String licensedTo) {
		Pattern emailPattern = Patterns.EMAIL_ADDRESS; // API level 8+
		Account[] accounts = AccountManager.get(appContext).getAccounts();
		for (Account account : accounts) {
		    if (emailPattern.matcher(account.name).matches()) {
		        String possibleEmail = account.name;
		        if(possibleEmail.equalsIgnoreCase(licensedTo))
		        	return true;
		    }
		}
		return false;
	...
	private static String getWhomLicenseTo() {
		String packagePath = fullVersionUri;
	    String classPath = packagePath+".MainActivity";
	    ...
	    try {
	        ...
	        /*Method method = handler.getDeclaredMethod("printIt", noparams);
			method.invoke(obj, null);*/
	        Field[] fields = handler.getDeclaredFields(); //does not include fields declared in Shape
	        for (Field f : fields) {
	            if(f.getName().equalsIgnoreCase("license")) {
	            	LicensedTo = f.get(obj).toString();
	            }  	
	        }	        
